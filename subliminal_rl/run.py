...
cs.store(group="model", name="mlp_aux_schema", node=ModelConfig)
...
cs.store(group="reward", name="aux_mse_schema", node=RewardConfig)
...
def main(cfg):
    ...
    print(f"  Aux dim: {config.model.aux_dim}")
    print(f"  Filler density: {config.env.filler_density}")
    print(f"  Noise input: {config.experiment.noise_input}")
    print(f"  Student method: {config.experiment.student_method}")
    print(f"  Distill batch size: {config.experiment.distill_batch_size}")
