...
def run_experiment(seed: int, config: Config, controls: set[str] | None = None) -> tuple[dict, dict]:
    ...
    vec_env_a = make_vec_env_a(
        ...
        wall_density=config.wall_density,
        max_steps=config.max_episode_steps,
        goal_rewards=red_teacher_rewards,
        base_seed=seed,
        filler_density=config.filler_density,
    )
    eval_seed = seed
    eval_fn = make_eval_fn(config, device, np.random.default_rng(eval_seed), torch_seed=seed)
    red_teacher, teacher_log = train_ppo(
        copy.deepcopy(theta_0),
        vec_env_a,
        config.teacher_total_steps,
        config,
        label="RED teacher",
        eval_fn=eval_fn,
        device=device,
    ...
    if "c3" in controls:
        ...
        vec_env_a_sym = make_vec_env_a(
            ...
            wall_density=config.wall_density,
            max_steps=config.max_episode_steps,
            goal_rewards=sym_rewards,
            base_seed=seed,
            filler_density=config.filler_density,
        )
        eval_fn_sym = make_eval_fn(config, device, np.random.default_rng(eval_seed), torch_seed=seed)
    ...
    if "c4" in controls:
        ...
        vec_env_a_blue = make_vec_env_a(
            ...
            wall_density=config.wall_density,
            max_steps=config.max_episode_steps,
            goal_rewards=blue_rewards,
            base_seed=seed,
            filler_density=config.filler_density,
        )
        eval_fn_blue = make_eval_fn(config, device, np.random.default_rng(eval_seed), torch_seed=seed)
